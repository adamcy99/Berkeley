...
def plot_pareto(lot_wafer,block_so,yld_loss,n,sail,lots_rows,lots_indices):
  ...
  if lots_indices is None:
    lots = []
  else:
    tempdf = lot_df if lots_rows is None else pd.DataFrame(lots_rows) 
    lots = [tempdf['lot_id'][i] for i in lots_indices]

  # if wafers_indices is None:
  #   wafers = []
  # else:
  #   tempdf = w_df if wafers_rows is None else pd.DataFrame(wafers_rows)
  #   wafers = [tempdf['wafer_id'][i] for i in wafers_indices]
