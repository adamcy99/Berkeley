...
from ay_useful_functions import empty_figure
...
daysback = 365
Query = """
SELECT LEFT(wafer_id,5) AS lot_id_base, lot_id, wafer_id, family_code, MIN(tw.Last_test_date) AS date,
...
# Lot Level df
lot_df = w_df.groupby(['lot_id_base','lot_id','family_code']).agg({'date':'min', 
...
app.layout = html.Div(children=[
  # This part generates the interactive components for the table
    ...
    html.Div(id='datatable-interactivity-container',
      ...
      dt.DataTable(
          ...
          style_data_conditional=([
            ...
            } for (col, value) in lot_df.quantile(0.9).iteritems()
          ]+
          [
            {
              'if': {'filter_query':'{{{}}} <= {}'.format(col, value), 'column_id':col},
              'backgroundColor': BAD_RED
            } for (col, value) in lot_df.quantile(0.25).iteritems()
          ])
      )]
    ),
    html.Br(),
    ...
    html.Div(children=[
        ...
        html.Br(),
        html.Label('Sail Macros to Include'),
        dcc.Checklist(
            id='sail-checklist',
            options=[
          ...
          style_data_conditional=([
            {
              'if': {'filter_query':'{{{}}} >= {}'.format(col, value), 'column_id':col},
              'backgroundColor': GOOD_GREEN
            } for (col, value) in w_df.quantile(0.9).iteritems()
          ]+
          [
            {
              'if': {'filter_query':'{{{}}} <= {}'.format(col, value), 'column_id':col},
              'backgroundColor': BAD_RED
            } for (col, value) in w_df.quantile(0.25).iteritems()
          ])
      )
    ]
...
@app.callback(
    ...
    Input('table-id','derived_virtual_data'),
    Input('table-id','derived_virtual_selected_rows')]
)
def plot_pareto(lot_wafer,block_so,yld_loss,n,sail,table_rows,table_indices):
