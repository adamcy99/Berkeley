...
d <- d %>% 
  mutate(
    y_0 = 10 + 5 * I(age == 'old') + rnorm(n=n(), mean=0, sd=1)
    )
```
...
```{r clean up by rounding}
## Permit me to clean this up just a little bit, there are a lot of digits
## that are included in these variables. I'm going to limit to two places 
...
2. On average, will people who are older tend to have higher or lower potential outcomes to control (compared to people who are younger)? 
Higher

3. On average, will people who are older tend to have higher or lower potential outcomes to treatment (compared to people who are youngers)? 
Higher

4. **Most importantly**: If these are *potential outcomes* then, can we empirically observe any of these outcomes? 
You cannot see y0 and y1 for the same person. No we can't observe them. We can run a randomization, and only observe the recorded outcome. The outcome that we observer of the one person is y, not y0 or y1. 
