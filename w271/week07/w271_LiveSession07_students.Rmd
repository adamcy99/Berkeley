...
## Deterministic Linear Time Model
...
**In a break-out room, do the following exercises:**
  ...
  6. Plot the original series and the estimated trend
...
ggplot(data = dump, aes(x=seq(1, 512), y=value, colour=variable)) +
  ylab('Values') +
  geom_line(aes(y=linear.predict)) +
  geom_line(aes(y=quadratic.predict)) +
  geom_line(aes(y=exp.predict))
